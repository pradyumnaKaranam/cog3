...
def Ingestion(config_data, project_info):
	...
	input_data = open(config_data["project_name"] + "_TechNotesData.csv", 'r')
		...
		try:
			...
			if(line[5] != 'null'):
				...
				if(line[5] not in apar_dict.keys()):
					...
					question = MySQLdb.escape_string("[TechNotes:" + line[0] + "] - " + line[1].replace('\t', ' '))

					result_str = "<html>"
					result_str = result_str +  "<p><b>TechNote URL - &nbsp;</b>&nbsp;" 
					hyperlike_lists = line[2].split(',') 
					for hl in hyperlike_lists:
						result_str = result_str + "<a href=\"%s\" target=\"_blank\">%s</a>&nbsp;&nbsp;" %(hl,hl)
					...
					try:
					   ...
					   # Rollback in case there is any error
					   print "Error: in QA entry in error_code_msg " + line[0]
					   db.rollback()
				...
				question_state = 'APPROVED'
				question = MySQLdb.escape_string("[TechNotes] - " + line[1].replace('\t', ' '))
