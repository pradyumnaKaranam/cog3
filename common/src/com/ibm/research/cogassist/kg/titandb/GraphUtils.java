...
public class GraphUtils {
	...
	public static boolean isNeighbor(Vertex source, Vertex target, String projectid, String language){
		return isNeighbor(source, target, null, projectid, language);
	}
	
	public static boolean isNeighbor(Vertex source, Vertex target, String label, String projectid, String language){
		try{
			System.out.println("In try.. Source: "+source.getProperty("name")+"; Target: "+target.getProperty("name")+"; Label: "+label);
			//return new GremlinPipeline(source).both(label).has("name", target.getProperty("name")).has("projectid", projectid).has("language", language).hasNext();
			Iterable<Edge> edges = source.getEdges(Direction.BOTH);
			for (Edge edge : edges) {
				if (edge.getLabel().equals(label) || label == null) {
					Vertex targetNode = edge.getVertex(Direction.IN);
					if(!source.equals(edge.getVertex(Direction.OUT)))
